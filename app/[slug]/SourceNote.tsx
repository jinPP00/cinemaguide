  ...
  return (
    <section className="section" aria-labelledby="source">
      ...
      <dl className="source-list">
        <div>
          ...
          <dd>
            {checked ?? '확인 기록이 없습니다'}
            {needsRecheck && (
              <span className="source-flag"> 재확인 필요</span>
            )}
          </dd>
        </div>
        <div>
      ...
      <p className="source-caution">
        관람료·주차 조건·운영 정보는 극장 사정에 따라 변경될 수 있습니다.
        {needsRecheck
          ? ' 이 지점은 원본 확인이 끝나지 않은 항목이 있어, 방문 전 공식 페이지에서 다시 확인해 주세요.'
          : ' 방문 전 공식 페이지에서 최신 정보를 확인하시기 바랍니다.'}
      </p>

      {branch.officialUrl && (
        <p style={{ marginTop: 14 }}>
          {/* 인용한 원 출처로 보내는 링크라 nofollow를 붙이지 않는다.
              rel의 noopener·noreferrer는 새 탭 보안 때문에 유지한다. */}
          <a
          ...
          >
            {branch.name} {branch.brandName} 공식 페이지에서 최신 정보 확인
            <svg
              width="16"
              height="16"
              viewBox="0 0 24 24"
